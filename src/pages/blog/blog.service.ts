...
export const blogAPI = createApi({
  reducerPath: 'blogAPI', // Tên fields trong redux State
  tagTypes: ['Posts'], // những kiểu tag cho phép dùng trong blog api, giúp quản lý ép 1 api nào đó gọi lại
  ...
  endpoints: (build) => ({
    ...
    addPost: build.mutation<IPost, Omit<IPost, 'id'>>({
      query: (body) => ({
        url: `posts`,
        method: 'POST',
        body: body
       ...
       */
      invalidatesTags: (result, error, arg) => [{ type: 'Posts' as const, id: 'LIST' }]
    })
  })
})
